 ...
 */
public class ConfluentStreamingSourceStateStoreFailureTest extends ConfluentStreamingTestBase {
  ...
  public void setUp() {
    ...
    topic = ConfluentStreamingSourceStateStoreFailureTest.class.getSimpleName() + "_" + testName.getMethodName();
  ...
  public void testConfluentStreamingSource() throws Exception {
    ...
    programManager.start(ImmutableMap.of(
      "cdap.streaming.maxRetryTimeInMins", "1",
      "cdap.streaming.baseRetryDelayInSeconds", "60"));
    programManager.waitForRun(ProgramRunStatus.RUNNING, 2, TimeUnit.MINUTES);

    // Should fail after retry times out
    programManager.waitForRun(ProgramRunStatus.FAILED, 2, TimeUnit.MINUTES);
